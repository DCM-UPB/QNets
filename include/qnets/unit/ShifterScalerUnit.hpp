...
{
private:
    void _applyShiftScale()
    { // meant to be called in computeValues
        _v = (_v + _shift)*_scale;
        if (_v1d != nullptr) {
...
public:
    // Constructor
    explicit ShifterScalerUnit(const double shift = 0., const double scale = 1.)
    {
        _shift = shift;
        _scale = scale;
    }
    ...
    {
        setParamValue(params, "shift", _shift);
        setParamValue(params, "scale", _scale);
    };
