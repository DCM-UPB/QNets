...
public:
    ...
    explicit NNLayer(const int &nunits = 1, ActivationFunctionInterface * actf = std_actf::provideActivationFunction()){if (nunits>1) { construct(nunits, actf);
}}
    ...
    void deconstruct() override{FedLayer::deconstruct(); _U_nn.clear();}
