...
The library contains trainer classes with virtual base trainer class
\verb+NNTrainer+, which already provides the \verb+bestFit+ method to find the
best NN fit among several fits to target data.
Currently the only trainer implementation is the one using GSL nlinear
...
Leaving out the FFNN and data setup however, it comes down to the following:
\begin{lstlisting}
  ...
  NNTrainingConfig tconfig = {flag_r, flag_d1, flag_d2, lambda_r, lambda_d1, lambda_d2, maxn_steps, maxn_novali};

  // create trainer and find best fit
  NNTrainerGSL * trainer = new NNTrainerGSL(tdata, tconfig);

  // (optional) setup ffnn's internal normalization before fitting
...
\\Make sure that your \verb+ffnn+ has the necessary derivatives set before
training (to be safe, always set \verb+d1+,\verb+d2+,\verb+vd1+ and \verb+d1vd1+ (+ \verb+d2vd1+) depending on \verb+flag_d1+/\verb+flag_d2+).
...
\begin{lstlisting}
  gsl_multifit_nlinear_parameters your_gsl_parameters = gsl_multifit_nlinear_default_parameters();
...
network) support serialization of their restorable configuration (i.e. no
pointers) into simple strings, by a code system. It is explained in detail in
