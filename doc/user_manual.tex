...
\item the \emph{ReLU function}: $\reluaf(x) \equiv max(0, x)$ \\
Due to their efficiency and because their gradient doesn't vanish, Rectified Linear Units are widely used in deep neural networks. However, the function is not differentiable at x=0 and has flat derivatives elsewhere. 
\item the \emph{SELU function}: $\seluaf(x) \equiv \lambda * 
\begin{cases}
  x & \text{if $x>0$} \\
  \alpha * e^{x}- \alpha & \text{if $x<=0$}
\end{cases}$ \\
The Scaled Exponential Units (with two fixed hyper-parameters $\alpha$ and $\lambda$ to be set) solve some problems of ReLUs and can provide a self-normalizing network. But still they are not differentiable at x=0. 
